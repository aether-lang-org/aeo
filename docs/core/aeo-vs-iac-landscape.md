...
| **Config management** | Ansible, Chef, Salt, Puppet | converge a host's packages/files/services to a desired state, over ssh or an agent | aeo **subsumes the "make the host ready" part** — the agent *self-installs* its substrate on demand — but its unit is a *contained node*, not a mutated host. It doesn't push arbitrary file/package state; it stands up enclosures. |
