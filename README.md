...
(Linux), `jail` / `bhyve_vm` / `freebsd_vm` (FreeBSD). `bwrap` is the lightest
tier — an unprivileged bubblewrap sandbox (no root, no host setup); `nspawn` is a
systemd-nspawn system container (the systemd-native LXC peer). Block setters (one
arg per call — Aether is fixed-arity), grouped by what they declare:
